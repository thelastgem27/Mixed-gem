...
const ADMIN_ROLES: UserRole[] = [
  "MINISTRY_ADMIN", "REGION_ADMIN", "ZONE_ADMIN", "WOREDA_ADMIN",
];
...
export async function requireAuth(req: AuthRequest, res: Response, next: NextFunction) {
  ...
  // Propagate DB errors — do not silently swallow them
  const [dbUser] = await db.select().from(users).where(eq(users.authUserId, authUser.id));
  if (dbUser) req.dbUser = dbUser;
  // If no dbUser found, req.dbUser stays undefined — onboarding endpoints handle this case
...
/**
 * Middleware that enforces the presence of a schoolId on the authenticated
 * user's profile. Returns 403 for school-level roles that lack a schoolId.
 * Admin roles (ministry/region/zone/woreda) pass through without a schoolId.
 */
export function requireSchool(req: AuthRequest, res: Response, next: NextFunction) {
  ...
  if (!ADMIN_ROLES.includes(role) && !req.dbUser.schoolId) {
    res.status(403).json({ error: "Forbidden: account has no school associated" });
    return;
  }
  next();
}
